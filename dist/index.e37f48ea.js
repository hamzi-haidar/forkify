...
const uploadRecipe = async function(newRecipe) {
    try {
        const ingredients = Object.entries(newRecipe).filter((entry)=>entry[0].startsWith("ingredient") && entry[1] !== "").map((ing)=>{
            ...
            if (ingArr.length !== 3) throw new Error("Wrong ingredient format! please use the correct fromat :)");
...
const KEY = "ed1ec483-e762-41f1-b99c-ba74615b5efb";
...
class RecipeView extends (0, _viewJsDefault.default) {
    ...
    _generateMarkupIngredient(ing) {
      ...
      <div class="recipe__quantity">${ing.quantity ? (0, _fractyDefault.default)(ing.quantity.toFixed(2)).toString() : ""}</div>
...
class PaginationView extends (0, _viewJsDefault.default) {
    ...
    _generateMarkup() {
        ...
        if (this._curPage === 1 && numPages > 1) return `<span class="pages-info">page ${this._curPage} of ${numPages}</span> ${this._generateMarkupButton("next")}`;
        // Last page
        if (this._curPage === numPages && numPages > 1) return `${this._generateMarkupButton("prev")}<span class="pages-info">page ${this._curPage} of ${numPages}</span>`;
        // Other page
        if (this._curPage < numPages) return `${this._generateMarkupButton("prev")}<span class="pages-info">page ${this._curPage} of ${numPages}</span> ${this._generateMarkupButton("next")}`;
